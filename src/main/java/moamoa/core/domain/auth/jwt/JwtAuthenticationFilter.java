package moamoa.core.domain.auth.jwt;
...
import moamoa.core.domain.auth.AuthDetails;
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        try {
            ...
            User user = om.readValue(request.getInputStream(), User.class); //클라이언트에서 전달된 JSON 데이터는 Member 클래스로 변환
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        Long id = authDetails.getUser().getId();
        String email = authDetails.getUser().getEmail();
