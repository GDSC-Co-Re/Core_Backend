...
@Tag(name = "메인 페이지 API", description = "메인 페이지 정보 제공용 API.")
@RequestMapping("/main")
public class MainInfoController {
    private final UserService userService; // UserService를 주입

    @GetMapping("/info")
    public ResponseEntity<MainInfoDto> getMainInfo(Authentication authentication) {
        // 인증된 사용자의 이메일 또는 ID를 가져옴
        String userEmail = authentication.getName(); // 또는 다른 식별 정보

        // UserService를 통해 필요한 정보 조회
        MainInfoDto mainInfoDto = userService.getMainInfo(userEmail);
